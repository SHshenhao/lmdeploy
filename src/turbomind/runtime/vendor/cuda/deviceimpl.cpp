...
int getDeviceCount() {
  int num = -1;
  std::cout<<"vendor getDeviceCount"<<std::endl;
//   ::cudaGetDeviceCount(reinterpret_cast<int*>(&num));
...
EventStatus getEventStatus(deviceEvent_t event) {
    ...
    if (ret == ::cudaSuccess) {
        ...
        VENDOR_CHECK(false, "unexpected event status in getEventStatus, ret = ", ret);
    }
}
...
OpStatus mallocDevice(void **p, size_t nbytes, bool throwExcepion) {
    ...
    if (r != ::cudaSuccess) {
        if(throwExcepion) {
            ::cudaGetLastError(); /* reset internal error state*/
            VENDOR_CHECK(false, "alloc failed in mallocDevice, ret = ", r, " size= ", nbytes);
        }
