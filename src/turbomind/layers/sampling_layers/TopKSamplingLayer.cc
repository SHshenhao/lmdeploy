...
void TopKSamplingLayer<T>::setup(const size_t batch_size, const size_t beam_width, TensorMap* runtime_args)
{
    ...
    diopiSetupTopkRuntimeArgsInp(&ctx_, top_ks, top_ps, skip_decode, batch_size, top_k, runtime_top_k_size, top_p, runtime_top_p_size);
    sync_check_cuda_error();
    dipu::diopi_helper::clearDiopiContextAll(ctx_);
    // std::cout<<"diopiSetupTopkRuntimeArgsInp"<<std::endl;
...
void TopKSamplingLayer<T>::runSampling(TensorMap* output_tensors, TensorMap* input_tensors)
{
    ...
    turbomind::Tensor top_ks_tensor{MEMORY_GPU, TYPE_INT32, {int64_t(batch_size)}, runtime_top_k_buf_};
    diopiConstTensorHandle_t top_ks = dipu::diopi_helper::toDiopiTensorHandle(top_ks_tensor);
    turbomind::Tensor top_ps_tensor{MEMORY_GPU, TYPE_FP32, {int64_t(batch_size)}, runtime_top_p_buf_};
    diopiConstTensorHandle_t top_ps = dipu::diopi_helper::toDiopiTensorHandle(top_ps_tensor);
    turbomind::Tensor skip_decode_tensor{MEMORY_GPU, TYPE_BOOL, {int64_t(batch_size)}, skip_decode_buf_};
    diopiTensorHandle_t skip_decode = dipu::diopi_helper::toDiopiTensorHandle(skip_decode_tensor);
    ...
    // std::cout<<"dtype"<<dtype<<" sizeofT"<<sizeof(T)<<std::endl;
