...
class FusedDeepEpMoEBlockedF8Impl(TritonFusedMoEBlockedF8Impl):
    ...
    def __init__(self,
        ...
        self.token_dispatcher_for2mb = TokenDispatcherBuilder.build(
            group=ep_group,
            num_experts=self.num_experts,
            num_local_experts=self.num_experts // ep_size,
            hidden_size=hidden_dim,
    ...
    def forward_yield(self,
        ...
        if shared_experts is not None:
            ...
            if self.token_dispatcher_for2mb.get_shared_experts() is None:
        ...
        if len(tag) > 1 and tag[1].upper() == 'D':
            # yield for attn1, dis (+share), dis_wait, moe
            recv_hidden_states, tokens_per_expert, shared_states_indispatch = (
                yield from _token_dispatcher.dispatch_low_latency_yield(
                    hidden_states,
                    topk_ids,
                    topk_weights,
                    self.num_experts,
            ...
            if tokens_per_expert.sum().item() > 0:
                recv_hidden_states = self.experts.forward(recv_hidden_states, tokens_per_expert, gate_up_weights,
                                                          gate_up_scale, down_weights, down_scale)
            # yield for moe, comb, (+share) comb_wait, (+share) attn0
            out_states, shared_states_incomb = yield from _token_dispatcher.combine_low_latency_yield(
                recv_hidden_states, hidden_states, is_prefill, is_decoding, True)
            ...
            recv_hidden_states, recv_topk_ids, recv_topk_weights, tokens_per_expert, shared_states_indispatch = (
                yield from _token_dispatcher.dispatch_yield(hidden_states, topk_ids.to(torch.int64),
                                                            topk_weights.to(torch.float32), self.num_experts,
                                                            is_prefill, is_decoding))
            recv_hidden_states = self.experts.forward(recv_hidden_states, tokens_per_expert, gate_up_weights,
                                                      gate_up_scale, down_weights, down_scale)
            # yield for moe, comb, (+share) comb_wait, (+share) attn0
            out_states, shared_states_incomb = yield from _token_dispatcher.combine_yield(
                recv_hidden_states, hidden_states, is_prefill, is_decoding)
            shared_states = shared_states_indispatch if shared_states_indispatch is not None else shared_states_incomb
